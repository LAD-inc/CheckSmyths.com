...
function updateStock(productId, storeId)
{
	...
	{
		xmlreqs[pos].freed = 0;
		xmlreqs[pos].xmlhttp.open("GET",'StockCheckBackground.php?productId='+productId+'&storeId='+storeId,true);
...
function xmlhttpChange(pos, storeId) 
{
	if (typeof(xmlreqs[pos]) != 'undefined' && xmlreqs[pos].freed == 0 && xmlreqs[pos].xmlhttp.readyState == 4) 
	{
		if (xmlreqs[pos].xmlhttp.status == 200 || xmlreqs[pos].xmlhttp.status == 304) 
		{
			...
			else if (xmlreqs[pos].xmlhttp.responseText.search("Low Stock") >= 0)
			{
				document.getElementById(elementId.concat(storeId)).style.background= '#FBBA00';
			}
			else if (xmlreqs[pos].xmlhttp.responseText.search("Coming Soon") >= 0)
			{
				document.getElementById(elementId.concat(storeId)).style.background= '#FBBA00';
			}
			else
